...
{
    public class ReorderSelectItemCallbackHandler : ICallbackHandler
    {
        public string Key => CallbackKeys.ReorderSelectItem;
        ...
        public ReorderSelectItemCallbackHandler(
        ...
        public async Task HandleAsync(CallbackQuery query, string[] args, CancellationToken ct)
        {
            ...
            var itemIdStr = args[0];
            if (!int.TryParse(itemIdStr, out var itemId))
            {
                _logger.LogWarning("Invalid menuId format: {ItemIdStr}", itemIdStr);
            ...
            await _reorderSessionManager.UpdateAsync(userId, session);
