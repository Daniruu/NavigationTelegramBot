...
{
    public class ItemEditMessageSessionHandler : ISessionHandler
    {
        public string Action => SessionKeys.NavigationItemEditMessage;

        private readonly IUserRepository _userRepository;
        private readonly ITelegramMessageService _messageService;
        private readonly ILocalizationManager _localizer;
        private readonly ISessionManager _sessionManager;
        private readonly ILogger<ItemEditMessageSessionHandler> _logger;
        ...
        public ItemEditMessageSessionHandler(
            IUserRepository userRepository,
            ITelegramMessageService messageService,
            ILocalizationManager localizer,
            ISessionManager sessionManager,
            ILogger<ItemEditMessageSessionHandler> logger,
        ...
        public async Task HandleAsync(Message message, SessionData session, CancellationToken ct)
        {
            ...
            var translationKey = itemToEdit.MessageTranslationKey;
            if (translationKey == null) return;

            var languageCode = LanguageCodeHelper.FromTelegramTag(langStr);

            await _translationService.SetTranslationAsync(translationKey, languageCode, messageText);
